...
def get_useful_ifo(pro_id,save_path):
    ...
    if response.status_code == 200:
        ...
        try:
            if "shortVideoUrl" in res_json["base"]:
                data_raw["shortVideoUrl"] = res_json["base"]["shortVideoUrl"]
        except:
            print("video image bug")
        ...
        try:
            ...
            print("previewImages image bug")
        data_raw["previewImages_image"] = url_raw
        
        url_raw = []
        try:
            if f"{pro_id}" in res_json['images']['groups']:
                temp3 = res_json['images']['groups'][f"{pro_id}"]['detailImages']
                ...
                temp3 = res_json['images']['groups']['spu']['detailImages']
                for i in range(len(temp3)):
                    #print(temp3[i]["imageUrl"])
                    url["imageUrl"] = temp3[i]["imageUrl"]
                    url_raw.append(url.copy())
        except:
            print("detailImages image bug")
        data_raw["detailImages_image"] = url_raw
        #写入json
...
#def run():
def run(arg):
    source_path = f"/home/dengyiru/vip_crwal/id_shoe/{arg}.json"
    save_path = f"/home/dengyiru/vip_crwal/new_content_test/{arg}.json"
...
if __name__ == '__main__':
    ...
    #args = arg[:25]
    #args = arg[25:]
    args = [arg[2],arg[4],arg[5],arg[8],arg[41]]
    #print(args)        
    #args = [arg[10]]
    #print(args)
    pool.map(run, args) 
